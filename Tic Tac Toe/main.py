...
class TTT(tk.Tk):
	...
	def Start(self):
		...
		tk.Button(self, text="Restart", bg='red', fg='white', activebackground='blue3', activeforeground='white', command=self.Restart).grid(row=3, column=1)
	...
	def checkWinner(self, char): # Can be improved
		...
		((self.btns[0][2]["text"] == char) and (self.btns[1][1]["text"] == char) and (self.btns[2][0]["text"] == char))):
			...
			self.Winner("The match has been Drawn")

	def Winner(self, char):
		top = tk.Toplevel(self)
		top.title("Congratulations You Win")
		if char == "DRAW":
